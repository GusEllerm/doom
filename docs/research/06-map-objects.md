...
- Skill filter: baby->bit 1; nightmare->bit 4 (hard set only); easy/normal/hard -> `1<<(gameskill-1)` (sk_easy=0 gives 1<<... note gameskill enum sk_baby=0, sk_easy=1, sk_normal=2, sk_hard=3, sk_nightmare=4). **No health scaling happens here** — health scaling per skill lives in `P_SpawnMobj`? No: it lives in `P_SpawnThing`? It is actually in `P_SpawnPlayer`/none: in v1.10 the ONLY skill-related spawn adjustment is the options-bit filter above (grep: `spawnhealth` is used only in P_KillMobj gib test; there is no `*2`/`/2` health scaling at spawn). Baby-skill monster damage halving is at `P_DamageMobj` (`if (player && gameskill == sk_baby) damage >>= 1;`).
